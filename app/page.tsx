...
import { ArchiveBoxIcon, CalendarDaysIcon, ClockIcon, CubeIcon } from "@heroicons/react/24/outline"
import { AtSymbolIcon, BuildingLibraryIcon, MapPinIcon, BookOpenIcon } from "@heroicons/react/24/solid"
...
export default function Home() {
	return (
		<main className={clsx(
			...
			<section>
				<p className="text-lg md:text-2xl font-medium text-zinc-500">Art & Ceramics Studio in Brussels</p>

				{/* TODO: have the title to use Inter Display (2023-04-18: in beta) */}
				<h1 className="text-4xl md:text-6xl lg:text-7xl font-bold tracking-tighter my-6">Learn. Grow. Share.</h1>

				<p>Malina More Studio is a beautiful 180m<sup>2</sup> ceramics, arts & crafts studio in Brussels, next to Anderlecht City Hall.</p>
				<p>We host weekly ceramic classes, kiln rental, and can provide co-working and event space. To find out more, contact us at <Link href={mailto}>contact@malinamore.studio</Link>.</p>
			</section>
			...
			<section>
				<div className={clsx(
					...
					<Image src={Prime} unoptimized alt="A flower-shaped blue vase holding a white flower, next to a set of similar colored vases" />
					<Image src={Handed} unoptimized alt="A hand holding a stack of ceramic dishes" />
					<Image src={Crescents} unoptimized alt="Two crescent-shaped plates, behind a blurred green plant" />
					<Image src={Six} unoptimized alt="Six small colorful deep dishes" />
				</div>
			</section>

			<section id="what-we-do" className="bg-zinc-100 w-full !max-w-[100vw] child:max-w-5xl child:mx-auto">
				...
				<div className={clsx(
					...
					<div>
						...
						<p className="text-sm text-zinc-500 flex md:items-center gap-x-1">
							<ClockIcon /> each lesson lasts 2 ~ 2<span className="fraction">1/2</span> hours
						</p>
					...
					<div>
						...
						<ul className="list-dash">
							<li>Under 15cm<sup>3</sup>: 3 € / piece</li>
							<li>Under 30cm<sup>3</sup>: 6 € / piece</li>
						</ul>
						<p className="text-sm text-zinc-500">For larger pieces, bulk orders, and special firings, <Link href={mailto}>contact us</Link></p>
					</div>
				</div>
			</section>
			...
			<section id="about" className="!py-12">
				...
				<p>Malina More ASBL was founded by Anna Radovenchik, by her wish of sharing her admiration of art and her passion for porcelain.</p>
				<p>Based in the Brussels Capital region and co-managed by Anna and her partner Jerome, the ASBL's goals are to teach and showcase the arts of ceramics, gravure, photography, and other fine arts.</p>
				<p>Are you looking for work or volunteering opportunities, please <Link href={mailto}>reach out to us</Link>.</p>
			</section>

			<section id="contact" className="w-full !max-w-[100vw] bg-black text-white flex justify-center !px-0">
				<div className="grid grid-flow-row-dense md:grid-flow-col-dense gap-x-8 gap-y-4 w-full max-w-5xl px-[5%]">
					<div>
						...
						<ul className="!pl-0 text-zinc-200 child:flex child:items-center child:gap-x-2">
							...
							<li>
								<AtSymbolIcon /> <Link href={mailto}>contact@malinamore.studio</Link>
							</li>
						</ul>
					</div>
					<div>
						...
						<ul className="!pl-0 grid grid-cols-2 grid-flow-dense text-zinc-200 child:flex child:items-center">
							...
							<li className="!inline-block">
								12:00—19:00 <span className="block gap-x-1"><BookOpenIcon /> classes only</span>
							</li>
